...
def keshihua(a,layer,image_num):
    try:
        print(a[0,:,:])
        a = np.array(a[0,:,:].cpu()).astype(int)
        inp=cv2.imread("/data/tzx/snake_envo_num/visual_result/dark1.png")
        #a=np.array(layer2_sup_poly[0].cpu()).astype(int)
        fig, ax = plt.subplots(1, figsize=(20, 10))
        fig.tight_layout()
        ax.axis('off')
        ax.imshow(inp)
        ax.plot(a[:, 0], a[:, 1],"o", color='white', linewidth=10,markersize=10)
        plt.savefig("/data/tzx/snake_envo_num/visual_result/中间可视化/{}_{}.png".format(image_num, layer), bbox_inches='tight', pad_inches=0)
    except:
        print(1)
...
class Evolution(nn.Module):
    ...
    def evolve_poly(self, snake, cnn_feature, i_it_poly, c_it_poly, ind):
        #cnn_feature.shape [bs, 64, 128, 128])
        if len(i_it_poly) == 0:
            return torch.zeros_like(i_it_poly)
        h, w = cnn_feature.size(2), cnn_feature.size(3)
        init_feature = snake_gcn_utils.get_gcn_feature(cnn_feature, i_it_poly, ind, h, w)
        #print('init_feature',init_feature.shape)
        c_it_poly = c_it_poly * snake_config.ro
        #print(c_it_poly)
        init_input = torch.cat([init_feature, c_it_poly.permute(0, 2, 1)], dim=1) #shape: bs, 66, 128
        adj = snake_gcn_utils.get_adj_ind(snake_config.adj_num, init_input.size(2), init_input.device) #shape: bs,128,4
        i_poly = i_it_poly * snake_config.ro + snake(init_input, adj).permute(0, 2, 1) #shape: bs,128,2
        #print('i it poly:', i_it_poly[0])
        #print('snake ', snake(init_input, adj).permute(0, 2, 1)[0])
        return i_poly
