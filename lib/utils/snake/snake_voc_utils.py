...
import math
import sys
sys.path.append("/data/tzx/AAtoolbox")
...
def augment(img, mask, split, _data_rng, _eig_val, _eig_vec, mean, std, polys=None):
    # resize input
    ...
    mask = cv2.warpAffine(mask, trans_input, (input_w, input_h), flags=cv2.INTER_LINEAR)
    ...
    #print("inp_out_hw",inp_out_hw)
    return orig_img, mask, inp, trans_input, trans_output, flipped, center, scale, inp_out_hw
