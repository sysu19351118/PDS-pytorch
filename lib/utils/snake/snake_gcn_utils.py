...
def prepare_testing_init(box, score):
    ...
    if score[0,0]<cfg.ct_score and cfg.replace_box:
       box[0,0,0]=30
       box[0,0,1]=30
       box[0,0,2]=98
       box[0,0,3]=98
       score[0,0]=cfg.ct_score+0.01
...
def prepare_training(ret, batch):
    ...
    if snake_config.train_pred_box:#不走这句
...
def get_gcn_feature(cnn_feature, img_poly, ind, h, w):
    ...
    for i in range(batch_size):
        ...
        #print(poly)
        feature = torch.nn.functional.grid_sample(cnn_feature[i:i+1], poly, mode='bilinear',align_corners=True)[0].permute(1, 0, 2)
        #print(feature.shape)
        #sys.exit(0)
